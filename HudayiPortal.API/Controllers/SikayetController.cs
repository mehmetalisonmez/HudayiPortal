...
using HudayiPortal.Application.Features.Sikayetler.Queries.GetMySikayetler;
using HudayiPortal.Application.Features.Sikayetler.Queries.GetSikayetById;
...
public class SikayetController : ControllerBase
{
	...
	[ProducesResponseType(typeof(List<SikayetDto>), StatusCodes.Status200OK)]
	[ProducesResponseType(StatusCodes.Status401Unauthorized)]
	public async Task<IActionResult> GetMySikayetler(CancellationToken cancellationToken)
	{
		var result = await _mediator.Send(new GetMySikayetlerQuery(), cancellationToken);
		return Ok(result);
	}

	[HttpGet("{id:int}")]
	[Authorize(Roles = "Admin,Personel")]
	[ProducesResponseType(typeof(SikayetDto), StatusCodes.Status200OK)]
	[ProducesResponseType(StatusCodes.Status404NotFound)]
	[ProducesResponseType(StatusCodes.Status401Unauthorized)]
	public async Task<IActionResult> GetById(int id, CancellationToken cancellationToken)
	{
		var result = await _mediator.Send(new GetSikayetByIdQuery(id), cancellationToken);
		if (result is null)
			return NotFound();
		return Ok(result);
	}
