...
using HudayiPortal.Application.Features.Duyurular.Queries.GetDuyurular;
using MediatR;
...
public class DuyuruController : ControllerBase
{
	...
	[HttpGet]
	[ProducesResponseType(typeof(List<DuyuruListDto>), StatusCodes.Status200OK)]
	[ProducesResponseType(StatusCodes.Status401Unauthorized)]
	public async Task<IActionResult> GetAll(CancellationToken cancellationToken)
	{
		var result = await _mediator.Send(new GetDuyurularQuery(), cancellationToken);
		return Ok(result);
	}
}
